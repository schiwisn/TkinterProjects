#--------------------------------------------------------------------------
# Simple food tracking application using Tkinter
...
class Application(Frame):
    """
    Application that keeps track of food items for a given day
    and commits these lists to a collection for further use.
    """

    def __init__(self, master=None):
        self.cur_file = "current.txt"
        self.col_file = "collection.txt"
        self.date = datetime.datetime.now().strftime("%d.%m.%Y")
        self.list_date = ""
        
        Frame.__init__(self, master)
        # Add
        Label(master, text="Food: ").grid(row=0, column=0)
    ...
    def commit(self):
        """Add current list to the collection
        and create new list initialised with the next date.
        """
        temp = []
        ...
        # Delete content of current file and add date of next day    
        next_day = datetime.datetime.now() + datetime.timedelta(days=1)
        # If the list date is not today use today as the next date
